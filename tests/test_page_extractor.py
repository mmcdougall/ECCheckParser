...
class TestPageExtractor(unittest.TestCase):
    def test_extract_range_august(self):
        src = ORIGINALS_DIR / 'city-council' / '2025' / 'agenda-packets' / '2025-08-19 Agenda Packet.pdf'
    ...
    def test_extract_range_february(self):
        src = ORIGINALS_DIR / 'city-council' / '2025' / 'agenda-packets' / '2025-03-18 Agenda Packet.pdf'
        self.assertTrue(src.exists(), f"Missing original PDF: {src}")
        with tempfile.TemporaryDirectory() as tmpdir:
            out = Path(tmpdir) / 'register.pdf'
            start, end = extract_check_register_pdf(src, out)
    ...
    def test_extract_range_october_payment_register(self):
        src = ORIGINALS_DIR / 'city-council' / '2025' / 'agenda-packets' / '2025-10-21 Agenda Packet.pdf'
        self.assertTrue(src.exists(), f"Missing original PDF: {src}")
        with tempfile.TemporaryDirectory() as tmpdir:
            out = Path(tmpdir) / 'register.pdf'
            start, end = extract_check_register_pdf(src, out)
